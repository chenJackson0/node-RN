...
function editM(req, res) {
  registeredUser.updateOne({ userName: req.body.userName }, {
    $set: {
    ...
    if (err) {
      ...
      publishedBase.update({ userName: req.body.userName }, {
        $set: {
          "perImg": req.body.img, "nickName": req.body.nickName
        }
      }, function (err, doc) { })
      //用户编辑个人信息时,修改之前主播的呢称
      registeredUser.updateMany({ "focusOns.name": req.body.userName }, {
        $set: {
      ...
      //用户编辑个人信息时,修改之前粉丝的呢称
      registeredUser.updateMany({ "fensi.name": req.body.userName }, {
        $set: {
...
fensi = (req) => {
  var data = {}
  registeredUser.updateOne({ userName: req.body.fensiUserName }, {
    $set: {
...
router.post('/focusOn', function (req, res, next) {
  registeredUser.updateOne({ userName: req.body.focusOnUserName }, {
    $set: {
